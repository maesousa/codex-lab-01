...
function totalsForMonth(items, month) {
  ...
  const expenses = inMonth.filter((x) => x.type === "expense").reduce((a, x) => a + (Number(x.amount) || 0), 0);
  const income = inMonth.filter((x) => x.type === "income").reduce((a, x) => a + (Number(x.amount) || 0), 0);
